         ...
         <div className="w-[85%] mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between h-16 items-center">
               <NavLink to={"/"} className="flex-shrink-0 flex items-center">
                  ...
                  <span className="ml-2 text-xl font-bold text-gray-900 dark:text-white">Country App</span>
               </NavLink>
               <div className="hidden md:flex space-x-4">
                  <NavLink to={"/"} className="text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 transition" onClick={() => setRegion("")}>
                     All
                  </NavLink>
                  {uniqueRegions.map((region) => (
                     <NavLink to={`/region/${region}`} key={region} className="text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 transition" onClick={() => setRegion(region)}>
                        {region}
                     </NavLink>
                  ))}
               </div>
               <div className="flex items-center">
                  <button className="text-gray-800 dark:text-gray-200 transition mr-4" onClick={handleTheme}>
